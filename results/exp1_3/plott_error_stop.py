...
try:
    ...
    if stop_df.empty:
        ...
        plt.tight_layout()
        plt.savefig('stop_phase_error_boxplot.png') # Speichert die Datei
        # plt.show() # <-- Fügen Sie dies hinzu, um das Fenster zu öffnen

        # --- Plot 2 ---
        plt.figure(figsize=(10, 10))
        gt_stops = stop_df.drop_duplicates('stop_number')[['stop_number', 'gt_pos_x', 'gt_pos_y']]
        sns.scatterplot(data=stop_df, x='pos_x', y='pos_y', hue='stop_number', palette='deep', s=15, alpha=0.7)
        sns.scatterplot(data=gt_stops, x='gt_pos_x', y='gt_pos_y', color='red', marker='X', s=200, edgecolor='black', label='Ground Truth Position')
        ...
        plt.axis('equal') 
        plt.tight_layout()
        plt.savefig('stop_phase_position_scatter.png') # Speichert die Datei
        # plt.show() # <-- Fügen Sie dies hinzu, um das Fenster zu öffnen

        # --- Plot 3 ---
        plt.figure(figsize=(15, 7))
        sns.scatterplot(data=stop_df, x='time_s', y='euclidean_error', hue='stop_number', palette='deep', s=10, alpha=0.6, edgecolor=None)
        ...
        plt.tight_layout()
        plt.savefig('stop_phase_error_over_time.png') # Speichert die Datei
        # plt.show() # <-- Fügen Sie dies hinzu, um das Fenster zu öffnen
        ...
        # ODER: Alle Plots am Ende anzeigen
        print("\nAlle Plots werden jetzt angezeigt...")
        plt.show() # Dieser Befehl öffnet alle erstellten "Figures"
